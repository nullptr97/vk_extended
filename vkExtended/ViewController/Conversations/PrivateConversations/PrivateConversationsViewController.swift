...
class PrivateConversationsViewController: BaseViewController, PrivateConversationsViewProtocol {
    ...
    var conversations = try! Realm().objects(Conversation.self).sorted(byKeyPath: "lastMessage.dateInteger", ascending: false).sorted(byKeyPath: "isImportantDialog", ascending: false).filter("isPrivateConversation = %@", true).filter("ownerId = %@", Constants.currentUserId)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        setNavigationItems(rightNavigationItems: [UIBarButtonItem(image: UIImage(named: "dismiss_24")?.withRenderingMode(.alwaysTemplate).tint(with: .systemBlue), style: .plain, target: self, action: #selector(self.onClosePrivateConversations))])
    }
    ...
    override func setNavigationItems(leftNavigationItems: [UIBarButtonItem] = [], rightNavigationItems: [UIBarButtonItem] = []) {
        super.setNavigationItems(leftNavigationItems: leftNavigationItems, rightNavigationItems: rightNavigationItems)
    }
    ...
    func observeConversations() {
        token = conversations.observe { [weak self] changes in
            ...
            self.footerView.footerTitle = self.conversations.count > 0 ? "\(self.conversations.count) \(getStringByDeclension(number: self.conversations.count, arrayWords: Localization.instance.privateConversationsCount))" : "Нет приватных чатов"
        }
    }
    ...
    func prepareTable() {
        ...
        mainTable.autoPinEdge(.top, to: .bottom, of: appBarViewController.headerView)

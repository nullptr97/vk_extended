...
/// VKExtended entry point
public final class VK {
    public static func setUp(appId: String, delegate: ExtendedVKDelegate, bundleName: String? = nil, configPath: String? = nil) {
    ...
    /// Returns VKExtended user sessions
    public static var sessions: SessionsHolder {
        guard let sessionsHolder = dependencies?.sessionsHolder else {
            fatalError("You must call VK.setUp function to start using VKExtended!")
        }
    ...
    /// Free up all VKExtended's resources and release memory

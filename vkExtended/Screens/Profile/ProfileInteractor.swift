...
class ProfileInteractor: ProfileInteractorProtocol {
    ...
    func makeRequest(request: ProfileModel.Request.RequestType) {
        ...
        case .getProfile(userId: let userId):
            ...
            if let profileCache = cache["profile_from_id\(userId)"] as Data? {
                let response = (ProfileResponse(from: profileCache.json()["profile"].arrayValue.first!), PhotoResponse(from: profileCache.json()["photos"]), FriendResponse(from: profileCache.json()["friends"]))
                
                friendResponse = response.2
                if response.0.canAccessClosed {
                    let cache = try! Cache<NSData>(name: "wallCache")
                    if let wallCache = cache["wall_from_id\(userId)"] as Data? {
                        let response = WallResponse(from: wallCache.json())
                        
                        wallResponse = response
                        presenter?.presentData(response: .presentProfileWall(wall: response, revealdedPostIds: self.revealedPostIds))
                    }
                }
                presenter?.presentData(response: .presentProfile(profile: response.0, photos: response.1, friends: response.2))
            } else {
            ...
            if let profileGetterPath = Bundle.main.path(forResource: "ProfileGetter", ofType: "txt") {
                do {
                    ...
                    try! Api.Execute.getProfilePage(code: code).done { [weak self] response in
                        ...
                        if response.0.canAccessClosed {
                            try! Api.Execute.wallGetWrapNew(ownerId: userId).done { [weak self] response in
                                guard let self = self else { return }
        ...
        case .getFriends(userId: let userId):
            ...
            try! Api.Execute.profileFriendsBlock(userId: userId, offset: countFriendOffset).done { [weak self] response in
                guard let self = self else { return }

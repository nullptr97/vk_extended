...
public class Cache<T: NSCoding> {
    ...
    private let queue = DispatchQueue.init(label: "com.aschuch.cache.diskQueue") // dispatch_queue_create("com.aschuch.cache.diskQueue", DISPATCH_QUEUE_CONCURRENT)
    ...
    public func setObject(object: T, forKey key: String, expires: CacheExpiry = .Never) {
        ...
        queue.sync(flags: .barrier) {
            self.add(object: cacheObject, key: key)
        }
    }
    ...
    public func removeExpiredObjects() {
        queue.sync(flags: .barrier) {
            ...
            for key in keys {
                let possibleObject = self.read(key: key)
    ...
    public subscript(key: String) -> T? {
        ...
        set(newValue) {
            if let value = newValue {
                setObject(object: value, forKey: key)
            } else {
    ...
    private func add(object: CacheObject, key: String) {
        ...
        let path = urlForKey(key: key).path
    ...
    private func read(key: String) -> CacheObject? {
        ...
        if let object = cache.object(forKey: key as AnyObject) as? CacheObject {
            return object
        }
        let path = self.urlForKey(key: key).path
    ...
    private func removeFromDisk(key: String) {
        let url = self.urlForKey(key: key)
    ...
    private func urlForKey(key: String) -> URL {
        let k = sanitizedKey(key: key)
        return cacheDirectory
            .appendingPathComponent(k)
            .appendingPathExtension("cache") as URL
    }
    ...
    private func expiryDateForCacheExpiry(expiry: CacheExpiry) -> Date {
        switch expiry {
        case .Never:

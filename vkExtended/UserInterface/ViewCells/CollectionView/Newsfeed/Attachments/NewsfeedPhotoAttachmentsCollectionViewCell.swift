...
class NewsfeedPhotoAttachmentsCollectionViewCell: UICollectionViewCell, ListBindable {
    
    var photoAttachmentsCollectionView = NewsAttachmentsCollectionView()
    let counterView: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor.getThemeableColor(fromNormalColor: .black).withAlphaComponent(0.7)
        view.clipsToBounds = true
        return view
    }()
    let counterLabel: UILabel = {
        let label = UILabel()
        label.textColor = .getThemeableColor(fromNormalColor: .white)
    ...
    override func awakeFromNib() {
        ...
        photoAttachmentsCollectionView.autoPinEdgesToSuperviewEdges(with: .top(8))
    ...
    func bindViewModel(_ viewModel: Any) {
        ...
        counterView.isHidden = viewModel.photoAttachements.count == 1
    }
}
